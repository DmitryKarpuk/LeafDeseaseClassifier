...
def train(
    image_path: Path, metadata_path: Path, model_path: Path, param_path: Path
) -> None:
    """
    ...
    train_ds = get_dataset(
        image_path, metadata_path, augmentation, mode="train"
    )
    # Create base model
    base_model = ResNet50(
        weights="imagenet", input_shape=size, include_top=False
    )
